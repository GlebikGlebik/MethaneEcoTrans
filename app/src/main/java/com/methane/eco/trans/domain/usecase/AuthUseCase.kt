...
class AuthUseCase (private val _authRepository: AuthRepository) {
    ...
    suspend operator fun invoke(email: String, password: String): EnterResult {

        // 1. БИЗНЕС-ПРАВИЛО: Валидация пустых полей
        if (email.isBlank() || password.isBlank()) {
            return EnterResult.Error("Необходимо заполнить все поля")
        }

        // 2. БИЗНЕС-ПРАВИЛО: Валидация формата email
        if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            return EnterResult.Error("Неверный формат email")
        }
